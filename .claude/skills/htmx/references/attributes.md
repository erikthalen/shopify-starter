...
<button hx-get="/data" hx-target="find .content">
  First .content descendant
</button>
<button hx-get="/data" hx-target="next .sibling">Next .sibling in DOM</button>
<button hx-get="/data" hx-target="previous .sibling">
  Previous .sibling in DOM
</button>
```
...
<button hx-get="/page" hx-select="#results" hx-select-oob="#notification">
  Load
</button>
```
...
<div hx-get="/api" hx-headers='js:{"Authorization": "Bearer " + getToken()}'>
  Load
</div>
```
...
<button hx-post="/action" hx-disabled-elt="closest form, #other-btn">
  Submit
</button>
```
...
<div
  hx-get="/data"
  hx-on:htmx:config-request="event.detail.parameters.extra = 'value'"
>
  Load
</div>
```
