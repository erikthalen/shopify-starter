...
```liquid
{{ variable }} # Output variable {{ variable | escape }} # Escape HTML
...
```liquid
{{ 'hello' | capitalize }} # Hello {{ 'HELLO' | downcase }} # hello
{{ 'hello' | upcase }} # HELLO {{ 'hello world' | truncate: 8 }} # hello...
{{ '<p>test</p>' | escape }} # &lt;p&gt;test&lt;/p&gt;
{{ 'hello world' | remove: 'world' }} # hello {{ 'hello' | append: ' world' }} #
hello world
```
...
```liquid
{{ collection | size }} # Number of items {{ collection | first }} # First item
{{ collection | last }} # Last item {{ collection | join: ', ' }} # Join with
comma {{ collection | reverse }} # Reverse order
...
```liquid
{{ 10 | plus: 5 }} # 15 {{ 10 | minus: 5 }} # 5 {{ 10 | times: 5 }} # 50
{{ 10 | divided_by: 5 }} # 2 {{ 10.5 | ceil }} # 11 {{ 10.5 | floor }} # 10
{{ 10.5 | round }} # 11
```
...
```liquid
{{ 1000 | money }} # $10.00 {{ 1000 | money_with_currency }} # $10.00 USD
...
```liquid
{{ shop.name }} # Store name {{ shop.email }} # Store email
{{ cart.item_count }} # Cart items {{ customer.name }} # Customer name (if
logged in) {{ settings.color_primary }} # Theme setting
```
